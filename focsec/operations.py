...
def get_ip_details(config, params):
    """
    Retrieves a list of all indicators from focsec.
    :param config: config
    :param params: params
    :return: List of all latest indicators from focsec Feed.
    """
    obj = Focsec(config)
    endpoint = '/v1/ip/{0}'.format(params.get('ip_address'))
...
def _check_health(config):
    try:
        obj = Focsec(config)
        obj.make_api_call(endpoint='/v1/ip/82.40.11.200', health_check=True)
        return True
